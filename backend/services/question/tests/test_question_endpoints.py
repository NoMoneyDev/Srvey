...
from services.question.schema import QuestionCreate


class TestQuestionEndpoints(unittest.IsolatedAsyncioTestCase):

    @patch("services.services.question.QuestionService.get_all_questions", new_callable=AsyncMock)
    async def test_get_all_questions_success(self, mock_get_all):
        ...
        from services.question.router import get_all_questions
    ...
    @patch("services.services.question.QuestionService.create_question", new_callable=AsyncMock)
    async def test_create_single_question_success(self, mock_create):
        ...
        from services.question.router import create_question
        payload = [
            QuestionCreate(
                survey_id="survey123",
                text="What is your name?",
    ...
    async def test_create_multiple_questions_success(self, mock_create):
        ...
        mock_questions = [
            MagicMock(id="q1", text="Question 1"),
            MagicMock(id="q2", text="Question 2"),
            MagicMock(id="q3", text="Question 3"),
        ]
        mock_create.side_effect = mock_questions

        from services.question.router import create_question
        payload = [
            QuestionCreate(survey_id="survey123", text="Question 1"),
            QuestionCreate(survey_id="survey123", text="Question 2"),
            QuestionCreate(survey_id="survey123", text="Question 3"),
        ]
        result = await create_question(question_data=payload)
    ...
    async def test_create_questions_preserves_data(self, mock_create):
        ...
        self.assertEqual(call_args.kwargs["text"], "What is your age?")
    ...
    async def test_create_questions_batch_processing(self, mock_create):
        ...
        from services.question.router import create_question
        payload = [
            QuestionCreate(survey_id="s1", text="Q1"),
            QuestionCreate(survey_id="s1", text="Q2"),
            QuestionCreate(survey_id="s1", text="Q3"),
        ]
        result = await create_question(question_data=payload)

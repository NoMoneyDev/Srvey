...
class TestUserService(unittest.IsolatedAsyncioTestCase):
    """Tests for UserService endpoints: /auth and /users"""
    ...
    async def test_google_auth_success_new_user(self, mock_create_user, mock_find_one, mock_verify_token):
        ...
        from routes.auth import google_auth
    ...
    @patch("routes.auth.id_token.verify_oauth2_token")
    @patch("routes.auth.UserService.find_one", new_callable=AsyncMock)
    async def test_google_auth_default_picture(self, mock_find_one, mock_verify_token):
    ...
    async def test_delete_user_success(self, mock_delete, mock_jwt_decode):
        ...
        from routes.user import delete_user
